...
require_once('admin/assets/func.php');
  $db = connect();
$news = mysqli_query($db, 'SELECT * FROM news where id_category not in (select id from categories where id = "11") order by id desc');
...
{
  echo '<url>'. PHP_EOL;
    echo '<loc>https://www.alnoortv.ma/news/'.cripter($rowNews['id'],264).'-'.replace($rowNews['titre']).'.html</loc>'. PHP_EOL;
    echo '<lastmod>'.date("Y-m-d").'</lastmod>'. PHP_EOL;
    echo '<priority>1.00</priority>'. PHP_EOL;
    echo '<image:image>'. PHP_EOL;
      echo '<image:loc>https://www.alnoortv.ma/assets/img/news/'.$rowNews['photo'].'</image:loc>'. PHP_EOL;
