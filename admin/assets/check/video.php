<?php
  ...
  if($_POST['act']=='addVideo'){
    if(($_POST['selectCategory'] == '')||(trim($_POST['titleVideo'])=='')||(trim($_POST['utlVideo'])=='')||($_POST['photo'] == '')||(!valide_url($_POST['utlVideo']))){
        ...
        (NULL,"'.$dr[0].'","'.$dr[1].'","'.GetIdUser($db).'","","'.addslashes($_POST['titleVideo']).'","'.$_POST['photo'].'","'.addslashes($description).'","","","","","'.$_POST['utlVideo'].'","","","'.rand(89, 568).'","","","","'.gmdate("Y/m/j H:i:s", time() + 3600*(1+date("I"))).'")');
          echo " addVideo ";
    }
    ...
    if(($_POST['selectCategory'] == '')||(trim($_POST['titleVideo'])=='')||(trim($_POST['url'])=='')||($_POST['photo'] == '')||(!valide_url($_POST['url']))){
      ...
      if($_POST['photo'] == ''){echo " ph ";}
        ...
        mysqli_query($db, 'UPDATE news SET id_category="'.$dr[0].'" , id_sousCategory="'.$dr[1].'" , titre="'.addslashes($_POST['titleVideo']).'", photo="'.$_POST['photo'].'" , description="'.addslashes($description).'" , urlVideo="'.$url.'"  where id ='.$_POST['id']);
        echo " modVideo ";
    }
